...
The rest of the dissertation is organized as follows. \Cref{sec:basic_form} introduces the basic formulation of video synopsis and details of the proposed tube rearrangement algorithm are described in \Cref{sec:proposed}. \Cref{sec:framework} contains explanations of other components that the online video synopsis consists of. \Cref{sec:exp} presents experimental results, and the dissertation is concluded in \Cref{sec:conc}.

\chapter{Basic formulation of video synopsis}
\label{sec:basic_form}

In this chapter, the basic formulation of video synopsis introduced in the pioneering works~\cite{Rav-Acha2006,Pritch2007,Pritch2008} is described to show which part of the formulation has to be changed for parallel processing. In addition, the reason why online video synopsis mainly considers collision energy is explained in detail.

As in \Cref{fig:video_synopsis,fig:video_synopsis_2d}, a principal objective of video synopsis is shortening length of the input video by relocating object tubes in temporal domain. In other words, we try to find the best combination of object tubes' starting positions in temporal domain (starting labels). In the field of video surveillance, a definition of the best combination can be different from specific applications. However, based on the paper of Pritch~\etal~\cite{Pritch2008}, the condensed video with the best starting label combination should have following characteristics.
\begin{itemize}
...
The last but not least RT is a metric to compare computational complexity of the algorithm and measured in milliseconds. This metric is important when the framework responds to the requests of users. Smaller RT is better for reducing a latency of the system. All experiments in following sections are conducted on a 4-core 4.0 GHz computer with 32 GB of memory. 
...
The experiments in this section are designed to 1) find optimum parameters for the proposed tube rearrangement algorithm, 2) prove the efficiency of the parallelized tube rearrangement, and 3) compare performances of several different online tube rearrangement algorithms.
...
\begin{figure}
	...
	\addlegendentry{Crossroad III}
	\addplot table [x expr=\coordindex, y=RT] {\tableLambdaTwo};
	\addlegendentry{Library lobby II}
	\addplot table [x expr=\coordindex, y=RT] {\tableLambdaThree};
	\addlegendentry{Subway station plaza II}
	\addplot table [x expr=\coordindex, y=RT] {\tableLambdaFour};
	\addlegendentry{Subway station plaza III}
	\addplot table [x expr=\coordindex, y=RT] {\tableLambdaFive};
	\addlegendentry{Subway station plaza IV}
	
	\end{axis}
	\end{tikzpicture}
	\caption{RTs measured by changing $\lambda$ for five sequences.}
	\label{fig:exp:lambda:RT}
\end{figure}

\subsubsection{Size of occupation matrix}
\label{sec:exp:occ}
Second experiment is conducted by changing spatial resolution $\mathcal{M}\times\mathcal{N}$ of the occupation matrix. Assume that the aspect ratio of the input video is 16:9 (the ratio of the width to the height). Based on the assumption, four candidates of $\mathcal{M}\times\mathcal{N}$ are considered in this experiment: $9\times16$, $18\times32$, $36\times64$, and $72\times128$. Remaining parameters are fixed as $\lambda=100$ and $K=20$ and the algorithm utilizes the binary occupation matrix. 

The larger binary occupation matrix has a better ability to encode the locations of the objects; therefore, the algorithm can finely adjust starting labels to avoid collisions. Then, the resulting video has lower FR and higher CR than the one using the smaller occupation matrix as shown in \Cref{fig:exp:occ}. On the other hand, reducing spatial resolution of the occupation matrix drastically increases the computation speed as depicted in \Cref{fig:exp:occ:RT}. Especially, according to \Cref{tb:exp:occ:RT}, the computation time is reduced by at least 1/40, when the width and height of the occupation matrix are scaled by 1/8 (from $72\times128$ to $9\times16$). Even though increasing $\mathcal{M}\times\mathcal{N}$ has definite advantages in FR and CR, degradation of the computation speed is too serious to be neglected. Therefore, the smallest resolution of the occupation matrix ($9\times16$) is utilized in this dissertation. 
...
\begin{figure}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{RTs measured by changing $\mathcal{M}\times\mathcal{N}$ for five sequences.}
	\label{fig:exp:occ:RT}
\end{figure}
\pgfplotstableset{
	create on use/Domain/.style={
		create col/set list={$9\times16$,$18\times32$,$36\times64$,$64\times128$}},
	columns/Domain/.style={string type},
}
\pgfplotstablenew[columns={Domain}]{4}\tableOccSzRT
\pgfplotstablecreatecol[copy column from table={\tableOccSzOne}{RT}]{VC7}{\tableOccSzRT}
\pgfplotstablecreatecol[copy column from table={\tableOccSzTwo}{RT}]{VC8}{\tableOccSzRT}
\pgfplotstablecreatecol[copy column from table={\tableOccSzThree}{RT}]{VC9}{\tableOccSzRT}
\pgfplotstablecreatecol[copy column from table={\tableOccSzFour}{RT}]{VC10}{\tableOccSzRT}
\pgfplotstablecreatecol[copy column from table={\tableOccSzFive}{RT}]{VC11}{\tableOccSzRT}
\begin{table}
	\centering
	...
	\caption{RTs measured by changing $\mathcal{M}\times\mathcal{N}$ for five video clips.}
	\label{tb:exp:occ:RT}
\end{table}

\subsubsection{Size of queue}
Adjusting the size of the queue $K$ determines how many object tubes are considered during each tube rearrangement step. The experiment is conducted as same manner as in \Cref{sec:exp:weight,sec:exp:occ}, while $K$ is changed from 10 to 100 with 10 interval. Other parameters are fixed to $\lambda=100$ and $\mathcal{M}\times\mathcal{N}=9\times16$, and the binary occupation matrix is used as a spatial approximation of the foreground pixels. Results of the experiments are presented in \Cref{fig:exp:queue,fig:exp:queue:RT}.

According to the result in \Cref{fig:exp:queue}, as $K$ increases, FR is reduced by at least 20\% and CR is slightly increased, while OR remains almost unchanged. Therefore, regarding the three metrics, large $K$ is beneficial for the online video synopsis framework. However, since the proposed algorithm takes more time in optimization for larger $K$, we should find a proper $K$ to make a balance between RT and other metrics. Empirically, $K$ is set as 50 and used for subsequent experiments.
...
\begin{figure}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{RTs measured by changing $K$ for five sequences.}
...
\subsection{Efficiency of parallelization}
To show the efficiency of our proposed parallelization scheme, parallelized and non-parallelized versions of the proposed algorithm are compared in terms of RT. A result of the comparison is illustrated in \Cref{fig:RT}. According to the results, the parallelized version is approximately 2.5 times faster than the non-parallelized version. Because the current parallelized version is based on CPU multi-threading, the proposed algorithm can be further efficiently computed with GPU-based parallelization.
