...
There are numerous choices for modeling the background, since this field of research has been studied for decades. According to Bouwmans~\etal~\cite{}, deep learning based background modelings can be categorized into three groups: deep encoder-decoder, convolutional neural networks, and generative adversarial network (GAN).

Even though deep learning based approaches show better foreground segmentation results than conventional statistical background modeling approaches~\cite{}, their computational burden are hard to be ignored. Therefore, in this dissertation, the proposed framework utilizes a well-known Gaussian mixture model~\cite{Zivkovic2004} to separate the foreground of the objects from the background. In addition, this modeling process can be accelerated by using the GPGPU.
\subsection{Object tube generation}
After the background modeling stage, we can get foreground masks of objects. To generate the object tubes, the masks that belong to the same object must be associated over temporal domain. For this task, simple yet efficient Hungarian algorithm~\cite{Kuhn1955} is utilized. 
